...
void Shredder()
{
  //Zwei beliebige Startpunkte wählen
  int A = Pick();
  int B = Pick();

  //Gesamten Text oft genug durcheinanderbringen
  int Count = (Zufallszahl() & 127) + 64;
  for (int i = 0; i < Count; i++)
  {
    //A und B sollen nicht gleich sein!
    if(A == B) { B = (A + 1) & 255; }

    //Hole CharA und CharB von den aktuellen Indizes
...
String Chiffre(String Source)
{
  ...
  String Ergebnis = Source; for(int k = 0; k < Windings; k++)
  {
    ...
    for (int Rune in Ergebnis.runes)
    {
      ...
      for(int f = 0; f < Count; f++) { Shredder(); Rune ^= Pick(); }
