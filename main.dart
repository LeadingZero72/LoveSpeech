...
int Berechnung(int Rune)
{
  ...
  for(int i = 0; i < Schleifen; i++)
  {
    ...
    //Beginne mit unsinnigen Werten
    int CharA = 0;
    int CharB = 0;

    //Bilde neue Werte, die jedenfalls NICHT 0 sind!
    while(CharA == 0) { CharA = (Passwort[A] + Pick()) & 255; }
    while(CharB == 0) { CharB = (Passwort[B] + Pick()) & 255; }
...
String Chiffre(String Source)
{
  ...
  if(Countrycode == 0) { Message('Windings $Windings, Wechsel $AlgorithmusWechsel, Chiffrierungen $Chiffrierungen, Picks $Picks'); Message('$Passwort'); Message('Passwortlänge = $PasswortLength'); Message('Schrumpfen = $Schrumpfen'); }
