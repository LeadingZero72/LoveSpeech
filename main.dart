...
class _ChiffrierungScreenState extends State<ChiffrierungScreen> with WidgetsBindingObserver
{
  ...
  List<int> PasswortAlgorithmen = List.filled(256, 0); //256 Byte lange Liste mit Passwort-Indizes
...
void AlgorithmusWahl()
{
  ...
  if(Schrumpfen == false)
  {
    ...
    if(PasswortLength < 256)
    {
      ...
      Passwort[PasswortLength] = Pick();

      //Der Algorithmus soll auch 256 Werte bekommen
      PasswortAlgorithmen[PasswortLength] = Pick();

      //Virtuelle Länge hat sich erhöht!
      PasswortLength++;
    }
    else { Schrumpfen = true; }
  }

  //Soll das Passwort schrumpfen ?
  if(Schrumpfen == true)
  {
    //Ist es noch länger als 128 Werte ?
    if(PasswortLength > 128)
    {
      PasswortLength--;
    }
    else { Schrumpfen = false; }
  }
...
void PasswortSchritt(int Step)
{
  ...
  PasswortIndex = (PasswortIndex + Step + PasswortCountdown + PasswortAlgorithmus + 1) % PasswortLength;
}
...
int Zufallszahl()
{
  //Es werden alle Passwortwerte und alle Algorithmen aufsummiert, um eine Zufallszahl zu erhalten, die vom Momentanzustand des Passworts und allen Algorithmen abhängig ist
  int Summe = 0;

  //Dafür müssen in einer Schleife alle Werte abgeklappert und summiert werden
  for(int i = 0; i < PasswortLength; i++)
  {
    Summe += Passwort[i] + PasswortAlgorithmen[i];
  }
...
int Berechnung(int Rune)
{
  ...
  int A = Pick() % PasswortLength;
  int B = Pick() % PasswortLength;

  //Erstmal sehen, wie lang das Passwort ist, davon die Hälfte nehmen
  int Halbwert = PasswortLength ~/ 2;
  ...
  for(int i = 0; i < Schleifen; i++)
  {
    ...
    if(A == B) { B = (A + 1) % PasswortLength; }

    //Bilde neue Werte, die jedenfalls NICHT 0 sind!
    int CharA = 0; while(CharA == 0) { CharA = (Passwort[A] + Pick()) & 255; }
    int CharB = 0; while(CharB == 0) { CharB = (Passwort[B] + Pick()) & 255; }
...
{
  //Konvertiere die Runen von Key (temporär) in eine Liste
  var Runen = Key.runes.toList();

  //Das ist die initiale Länge des Passworts
  PasswortLength = Runen.length;

  //Setze die ersten Einträge des Passworts
  Passwort.setRange(0, PasswortLength, Runen);
  ...
  PasswortAlgorithmen.setRange(0, PasswortLength, Passwort);

  //Zufälligen Wert für die Anfangszustände nehmen
  int Zauber = Zufallszahl() % PasswortLength;

  //Unfassbarer "letzter Passwort-Index-Slot"
  PasswortAlgorithmus = Passwort[Zauber] % PasswortLength;
  ...
  PasswortIndex = Passwort[(Zauber + 1) % PasswortLength] % PasswortLength;

  //Zu Beginn soll das Passwort wachsen, nicht schrumpfen!
  Schrumpfen = false;
...
String Chiffre(String Source)
{
  ...
  if(Style == 0) { Message('Windings $Windings, Wechsel $AlgorithmusWechsel, Chiffrierungen $Chiffrierungen, Picks $Picks'); Message('$Passwort'); Message('Passwortlänge = $PasswortLength'); Message('Schrumpfen = $Schrumpfen'); }

  //Passwort + Algorithmen unkenntlich machen
  //Passwort.fillRange(0, PasswortLength, 0); PasswortAlgorithmen.fillRange(0, PasswortLength, 0);
  Passwort.fillRange(0, 256, 0); PasswortAlgorithmen.fillRange(0, 256, 0);
...
//######################################################################################################
//#            Lädt ein kostenloses Zufallsbild aus dem Internet herunter
//######################################################################################################
/*
Uint8List? HintergrundDownload;

Future<void> HintergrundbildDownloaden() async
{
  try
  {
    //URL für ein zufälliges Bild von Picsum
    final response = await http.get(Uri.parse('https://picsum.photos/800/600'));
    if(response.statusCode == 200)
    {
      setState(() {
        HintergrundDownload = response.bodyBytes;
      });
    } else { throw Exception('Fehler beim Herunterladen des Bildes: ${response.statusCode}'); }
  } catch (e) {
    print('Fehler: $e');
  }
}
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        IconButton(
                          ...
                          onPressed: () {
                            if(Textfeld.text.isNotEmpty) { TranslateText(); }
                          },
                        ),
                        IconButton(
                          icon: Icon(Icons.message),
                          onPressed: () {
                            if(Textfeld.text.isNotEmpty) { ShowSMSDialog(); }
                          },
                        ),
                      ],
                    ),
