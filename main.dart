...
final Map<String, Map<String, String>> Sprachelemente =
{
  'en':
  {
    ...
    'Zwischenablage':   'Text copied to clipboard',
  },
  'de':
  {
    ...
    'Zwischenablage':   'Text in Zwischenablage kopiert',
  },
  'fr':
  {
    'Chiffrieren':      'Crypter',
    ...
    'Zwischenablage':   'Texte copié dans le presse-papiers',
  },
  'es':
  {
    'Chiffrieren':      'Cifrar',
    ...
    'Zwischenablage':   'Texto copiado en el portapapeles',
  },
  'it':
  {
    'Chiffrieren':      'Cripta',
    ...
    'Zwischenablage':   'Testo copiato negli appunti',
  },
};
...
{
  //Momentanzustand des Passworts bestimmen (der ändert sich ständig und ist nicht zurückverfolgbar)
...
int Wert()
{
  //Multidimensionalen Vektor-Hash ausrechnen (eine handvoll beliebiger Werte aus dem Passwort zusammenrechnen)
  for(int i = 0; i < Dimensionen; i++)
  {
    //Jede Dimension hat einen Index, der auf die aktuelle Länge des Passworts getrimmt wird
    int A = Index[i] % PasswortLength;

    //Der bestehende Hash wird mit dem Passwort-Zeichen, auf das der Index zeigt, weiter manipuliert
    Hash ^= Passwort[A];

    //Anschließend wird das Passwort manipuliert, wodurch es zu Overflow/Wrap-Around etc. kommt, d.h. es wird nicht zurückverfolgbar
    Passwort[A] = (Passwort[A] + Hash) & 255;

    //Der Index soll sich auf ähnliche Weise immer weiter verändern und nächstesmal auf ein anderes Passwortbyte zeigen
    Index[i] = (Index[i] + Hash) & 255;
  }
  //Der gebildete Hash-Wert liegt im Bereich 0..255 und wird als Wert zurückgegeben, um damit was auch immer zu tun..
  return Hash;
}


//#########################################################################################
//#                        ALGORITHMUS KÜNSTLICHE ALTERN LASSEN
//#########################################################################################
void Weisheit()
{
  ...
  while(Wert() > 127)
  {
    //Die Dimensionen des Zugriffsvektors ändern sich dabei auf unvorhersehbare Weise
...
//#########################################################################################
//#                  Zum Verschlüsseln brauche ich Werte ungleich Null
//#########################################################################################
int NichtNull()
{
...
{
  //Maximal 256 Verschlüsselungen für jede Rune!
  for(int a = 0; a < 256; a++)
  {
    ...
    //Die Passwortlänge ist unbekannt, variabel und daher auch die Schleifenzahl hier...
    if(a >= PasswortLength) break;
  }
...
String Chiffre(String Quelle)
{
  ...
  int Windings = (543 ~/ Runes.length) + Zufall(2); if(Windings < 3) { Weisheit(); Windings = 3 + Zufall(2); }
...
//######################################################################################################
//#                     Kompakte all-in-one-Verschlüsselungsroutine
//######################################################################################################
void Encrypt()
{
...
//######################################################################################################
//#                 Kompakte all-in-one-Entschlüsselungsroutine
//######################################################################################################
void Decrypt()
{                    
...
void ExportUserListToTextField()
{
  ...
  //Setze die Benutzerliste in das Textfeld und chiffriere, damit der Benutzer die Passwörter nicht sehen muß!
  setState(() { Textfeld.text = Buffer.toString(); isEncrypted = false; Encrypt(); });
}
...
{
  Decrypt(); String content = Textfeld.text.trim();
  ...
  if(Count > 0) { Message('$Count nicknames updated !'); }
}
                    ...
                    child: Column(
                      ...
                      children: [
                        IconButton(
                          ...
                          onPressed: () {
                            CopyToClipboard(Textfeld.text);
                            Message(Sprachelement('Zwischenablage'));
                          },
                        ),
                        IconButton(
