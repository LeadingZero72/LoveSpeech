...
String Chiffre(String Source)
{
  ...
  //Passwort/Algorithmen löschen und fertigen Text/Chiffre zurückgeben..
...
/*
List<T> SecureShuffle<T>(List<T> list)
{
  Random random = Random.secure();
...
String StarkesPasswort(int size)
{
  ...
  const String specialChars = '!()-=+{}:,.<>?ß°@§%';
  
  //Stelle sicher, dass mindestens ein Zeichen aus jeder Gruppe enthalten ist
  Random random = Random.secure();
  ...
  //Mische das Passwort, um die ursprüngliche Reihenfolge der Zeichen zu randomisieren
  //return (password.toString().split('')..shuffle()).join();
  return SecureShuffle(password.toString().split('')).join();
}
*/
...
bool SicheresPasswort(String Password)
{
  ...
  if(Password.length < 12) { Message('The password must be at least 12 characters long!'); return false; }
  ...
  if(Result == false) { Message('The password must contain at least one uppercase, one lowercase, a number and a special character and must be at least 12 characters long'); }
...
void ShowDeleteConfirmationDialog(BuildContext context, int index, String username)
{
  showDialog(
    ...
    builder: (BuildContext context) {
      return AlertDialog(
        backgroundColor: AppBarColor,
        title: Text('Delete Buddy', style: TextStyle(color: Textfarbe)),
...
void ShowEmailDialog()
{
  ...
  showDialog(
    ...
    builder: (BuildContext context) {
      return AlertDialog(
        title: Text('Send cipher email to...'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            TextField(
...
void ShowEmailDialog()
{
  ...
  showDialog(
    ...
    builder: (BuildContext context) {
      return AlertDialog(
        backgroundColor: AppBarColor,
        title: Text('Send cipher email to...', style: TextStyle(color: Textfarbe)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            TextField(
          ...
          TextButton(
            onPressed: () {
              ...
              if(to.isNotEmpty) {
                SendeEmail(to, subjectController.text.trim(), Textfeld.text.trim());
                Navigator.of(context).pop();
              } else { Message('Please fill in an email address.'); }
            },
            child: Text('Send Email', style: TextStyle(color: Textfarbe)),
          ),
        ],
      );
    },
